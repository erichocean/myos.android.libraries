...
{
    //printf("_cairo_scaled_font_init");
    cairo_status_t status;
...
cairo_scaled_font_create (cairo_font_face_t          *font_face,
			  ...
			  const cairo_font_options_t *options)
{
    //printf("cairo_scaled_font_create 1");
    cairo_status_t status;
    cairo_scaled_font_map_t *font_map;
    ...
    if (unlikely (status))
        return _cairo_scaled_font_create_in_error (status);
    //printf("cairo_scaled_font_create 2");
    ...
    //printf("cairo_scaled_font_create 3");
    ...
    //printf("cairo_scaled_font_create 4");
    if (scaled_font != NULL) {
        ...
        //printf("cairo_scaled_font_create 5");
        if (likely (scaled_font->status == CAIRO_STATUS_SUCCESS)) {
            /* We increment the reference count manually here, (rather
             * than calling into cairo_scaled_font_reference), since we
             * must modify the reference count while our lock is still
    ...
    //printf("cairo_scaled_font_create 6");
    
    /* Otherwise create it and insert it into the hash table. */
    if (font_face->backend->get_implementation != NULL) {
        //printf("cairo_scaled_font_create 6.1");
        font_face = font_face->backend->get_implementation (font_face,
                                                            font_matrix,
                                                            ctm,
                                                            options);
        //printf("cairo_scaled_font_create 6.11");
        if (unlikely (font_face->status)) {
            //printf("cairo_scaled_font_create 6.12");
            _cairo_scaled_font_map_unlock ();
            //printf("cairo_scaled_font_create 6.13");
            return _cairo_scaled_font_create_in_error (font_face->status);
        }
    }
    //printf("cairo_scaled_font_create 6.2");
    status = font_face->backend->scaled_font_create (font_face, font_matrix,
                                                     ctm, options, &scaled_font);
    /* Did we leave the backend in an error state? */
    if (unlikely (status)) {
        //printf("cairo_scaled_font_create 6.3");
        _cairo_scaled_font_map_unlock ();
        if (font_face != original_font_face)
            cairo_font_face_destroy (font_face);
        ...
        //printf("cairo_scaled_font_create 6.4");
        status = _cairo_font_face_set_error (font_face, status);
        return _cairo_scaled_font_create_in_error (status);
    }
    //printf("cairo_scaled_font_create 7");
    ...
    //printf("cairo_scaled_font_create 8");
    _cairo_scaled_font_map_unlock ();
    ...
    //printf("cairo_scaled_font_create 9");
    return scaled_font;
}
