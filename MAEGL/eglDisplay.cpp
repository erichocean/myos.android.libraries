...
eglDisplay::eglDisplay() :
    ...
    //m_attribs(android::DefaultKeyedVector<EGLint, EGLint>(ATTRIBUTE_NONE)),
    m_configs(NULL),
    m_gles_iface(NULL),
    m_gles2_iface(NULL),
...
EGLBoolean eglDisplay::getConfigAttrib(EGLConfig config, EGLint attrib, EGLint *value)
{
    ...
    //pthread_mutex_lock(&m_lock);
    //EGLBoolean ret = 0; //getAttribValue(config, m_attribs.valueFor(attrib), value);
    //pthread_mutex_unlock(&m_lock);
    //LOGE("eglDisplay::getConfigAttrib attrib: %d", attrib);
    //LOGE("eglDisplay::getConfigAttrib EGL_NATIVE_VISUAL_ID: %d", EGL_NATIVE_VISUAL_ID);
    switch (attrib) {
        case EGL_NATIVE_VISUAL_ID:
            *value = PIXEL_FORMAT_RGBA_8888;
            break;
        default:
            return false;
    }
...
EGLBoolean eglDisplay::setConfigAttrib(EGLConfig config, EGLint attrib, EGLint value)
{
    //Though it seems that valueFor() is thread-safe, we don't take chanses
    //pthread_mutex_lock(&m_lock);
    EGLBoolean ret = 0; //setAttribValue(config, m_attribs.valueFor(attrib), value);
    //pthread_mutex_unlock(&m_lock);
    return ret;
}
