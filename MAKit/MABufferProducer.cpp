...
namespace android {
    ...
    class MABpBufferProducer : public MABufferProducer
    {/*
    public:
    ...
    */
    ...
    MABufferProducer::QueueBufferInput::QueueBufferInput(const Parcel& parcel) {
        parcel.read(*this);
    }
    
    size_t MABufferProducer::QueueBufferInput::getFlattenedSize() const
    {
        return sizeof(timestamp)
        + sizeof(crop)
        + sizeof(scalingMode)
        + sizeof(transform)
        + 0;//fence->getFlattenedSize();
    }
    
    size_t MABufferProducer::QueueBufferInput::getFdCount() const
    {
        return 0;//fence->getFdCount();
    }
     
    status_t MABufferProducer::QueueBufferInput::flatten(void* buffer, size_t size,
                                                               int fds[], size_t count) const
    {
        ...
        err = 0;//fence->flatten(p, size - (p - (char*)buffer), fds, count);
        return err;
    }
    
    status_t MABufferProducer::QueueBufferInput::unflatten(void const* buffer,
                                                                 size_t size, int fds[], size_t count)
    {
        ...
        //fence = new Fence();
        err = 0;//fence->unflatten(p, size - (p - (const char*)buffer), fds, count);
        return err;
    }
