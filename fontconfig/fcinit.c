...
{
    printf("FcInitLoadOwnConfig");
    if (!config) {
        printf("FcInitLoadOwnConfig !config");
        config = FcConfigCreate();
        if (!config) {
            return NULL;
        }
    ...
    if (config->cacheDirs && config->cacheDirs->num == 0) {
        ...
        fprintf (stderr,
                 ...
                 FC_CACHEDIR);
        printf("Fontconfig warning: no <cachedir> elements found. Check configuration.\n");
        ...
        printf("FcStrFree(prefix)");
        FcStrFree(prefix);
    }
    printf("FcInitLoadOwnConfig end");
    return config;
}
